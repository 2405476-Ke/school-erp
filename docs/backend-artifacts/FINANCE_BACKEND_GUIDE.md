...
class AccountCategory(Base):
    __tablename__ = 'finance_account_categories'
...
class Account(Base):
    ...
    category_id = Column(UUID(as_uuid=True), ForeignKey('finance_account_categories.id'), nullable=False)
    parent_id = Column(UUID(as_uuid=True), ForeignKey('finance_accounts.id'), nullable=True)
...
class AccountRepository:
    ...
    async def has_transactions(self, account_id: UUID) -> bool:
        query = "SELECT 1 FROM finance_journal_lines WHERE account_id = :acc_id LIMIT 1"
...
class AccountingPeriod(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    year_id = Column(UUID(as_uuid=True), ForeignKey('finance_years.id'), nullable=False)
...
class PeriodService:
    ...
    async def close_period(self, period_id: UUID):
        period = await self.session.execute(select(AccountingPeriod).where(AccountingPeriod.id == period_id))
        period = period.scalars().first()
        if not period:
        ...
        # Validate no draft journals
        drafts = await self.session.execute(
            select(JournalEntry)
            .where(JournalEntry.period_id == period.id, JournalEntry.status == JournalStatus.DRAFT)
        )
        if drafts.scalars().first():
    ...
    async def open_next_period(self, current_period_id: UUID):
        # Implementation to locate the next chronological period and change status to OPEN
        pass
```
...
class JournalEntry(Base):
    ...
    period_id = Column(UUID(as_uuid=True), ForeignKey('finance_periods.id'), nullable=False)
...
class JournalLine(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    journal_id = Column(UUID(as_uuid=True), ForeignKey('finance_journal_entries.id'), nullable=False)
    account_id = Column(UUID(as_uuid=True), ForeignKey('finance_accounts.id'), nullable=False)
    cost_center_id = Column(UUID(as_uuid=True), ForeignKey('finance_cost_centers.id'), nullable=True)
...
class JournalService:
    ...
    async def post_journal(self, journal_id: UUID, user_id: UUID):
        result = await self.session.execute(
            select(JournalEntry)
    ...
    async def reverse_journal(self, journal_id: UUID, user_id: UUID):
        # Implementation to duplicate journal with negated lines, link, and post
        pass
```
...
class BudgetLine(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    budget_id = Column(UUID(as_uuid=True), ForeignKey('finance_budgets.id'))
    account_id = Column(UUID(as_uuid=True), ForeignKey('finance_accounts.id'))
    cost_center_id = Column(UUID(as_uuid=True), ForeignKey('finance_cost_centers.id'), nullable=True)
...
class BudgetService:
    ...
    async def check_budget_availability(self, account_id: UUID, cost_center_id: UUID, amount: Decimal, period_id: UUID):
        # 1. Get Budget
        budget_query = select(func.sum(BudgetLine.amount)).where(
            BudgetLine.account_id == account_id,
            BudgetLine.cost_center_id == cost_center_id
        )
        budget_res = await self.session.execute(budget_query)
        allocated_budget = budget_res.scalar() or Decimal('0.00')
        
        # 2. Get Actuals (Net movements)
        actuals_query = select(func.sum(AccountBalance.debit_movement - AccountBalance.credit_movement)).where(
            AccountBalance.account_id == account_id,
            AccountBalance.cost_center_id == cost_center_id
        )
        actuals_res = await self.session.execute(actuals_query)
        actual_spend = actuals_res.scalar() or Decimal('0.00')
        
        if (actual_spend + amount) > allocated_budget:
            raise InsufficientBudgetError("Transaction exceeds allocated budget for this account and cost center.")
```
