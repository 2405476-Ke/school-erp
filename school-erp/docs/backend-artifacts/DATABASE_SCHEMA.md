...
CREATE UNIQUE INDEX uq_student_active_enrollment
    ON student_class_enrollments(student_id, academic_year_id)
    WHERE status = 'ACTIVE';
...
CREATE TABLE staff (
    ...
    nhif_number VARCHAR(50), -- retained for legacy migration; new records populate shif_number
    shif_number VARCHAR(50), -- Social Health Insurance Fund (successor to NHIF, 2024)
...
CREATE TABLE payroll_entries (
    ...
    shif DECIMAL(15,4) NOT NULL DEFAULT 0, -- Social Health Insurance Fund (2.75% of gross)
    nhif DECIMAL(15,4) NOT NULL DEFAULT 0, -- retained for pre-2024 payroll history
...
CREATE TABLE fee_vote_heads (
    ...
    is_restricted BOOLEAN DEFAULT false, -- true = MOE capitation vote heads that cannot be re-appropriated
    allow_arrears_carry BOOLEAN DEFAULT true, -- if false, unpaid balances at term end are written off
    created_at TIMESTAMPTZ DEFAULT now(),
    UNIQUE(school_id, name)
);
...
CREATE TABLE book_loans (
    ...
    created_at TIMESTAMPTZ DEFAULT now(),
    CHECK ((student_id IS NOT NULL)::int + (staff_id IS NOT NULL)::int = 1)
);
```
...
CREATE TABLE gate_events (
    ...
    leave_pass_id UUID REFERENCES student_leave_passes(id),
    result VARCHAR(20) NOT NULL DEFAULT 'AUTHORIZED' CHECK (result IN ('AUTHORIZED', 'BLOCKED', 'OVERRIDE')),
    reason TEXT,
...
Ensures every journal entry obeys the double-entry accounting principle (Debits = Credits) whenever the entry is POSTED, whether the transition happens via INSERT or UPDATE.
...
BEGIN
    IF NEW.status = 'POSTED' THEN
        ...
        IF total_debit = 0 AND total_credit = 0 THEN
            RAISE EXCEPTION 'Journal entry % has no lines and cannot be POSTED.', NEW.entry_number;
        END IF;

        IF total_debit <> total_credit THEN
            RAISE EXCEPTION 'Journal entry % unbalanced. Total Debit: %, Total Credit: %',
...
-- Also block edits to journal_lines once the parent is POSTED.
CREATE OR REPLACE FUNCTION lock_posted_journal_lines()
RETURNS TRIGGER AS $$
DECLARE
...
DROP TRIGGER IF EXISTS lock_posted_lines ON journal_lines;
CREATE TRIGGER lock_posted_lines
BEFORE INSERT OR UPDATE OR DELETE ON journal_lines
FOR EACH ROW
EXECUTE FUNCTION lock_posted_journal_lines();
```
...
-- Concurrent refresh requires a unique index so it can be run without blocking readers.
CREATE UNIQUE INDEX IF NOT EXISTS uq_mv_payroll_summary
    ON mv_payroll_summary(payroll_period_id);
-- Refresh: REFRESH MATERIALIZED VIEW CONCURRENTLY mv_payroll_summary;
```

---

## 24. GOVERNANCE — DELEGATION OF AUTHORITY

Financial and disciplinary thresholds are stored in the database (not hard-coded in Python) so schools can tune them without a code release. The Delegation of Authority (DOA) table drives approval routing for requisitions, journals, leave, and disciplinary actions.

```sql
CREATE TABLE doa_thresholds (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    school_id UUID NOT NULL REFERENCES schools(id) ON DELETE CASCADE,
    domain VARCHAR(50) NOT NULL CHECK (domain IN (
        'PROCUREMENT', 'JOURNAL', 'PAYROLL', 'LEAVE', 'DISCIPLINARY'
    )),
    tier VARCHAR(50) NOT NULL, -- e.g., 'TIER_1', 'TIER_2', 'BOM'
    approver_role VARCHAR(50) NOT NULL, -- e.g., 'BURSAR', 'PRINCIPAL', 'BOM'
    min_amount DECIMAL(15,4) NOT NULL DEFAULT 0,
    max_amount DECIMAL(15,4),
    effective_from DATE NOT NULL DEFAULT CURRENT_DATE,
    effective_to DATE,
    created_at TIMESTAMPTZ DEFAULT now(),
    UNIQUE(school_id, domain, tier, effective_from)
);
COMMENT ON TABLE doa_thresholds IS 'Delegation of Authority thresholds per BRD Section 7.2. Amounts are inclusive of min_amount and exclusive of max_amount.';

CREATE TABLE approval_workflow_events (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    school_id UUID NOT NULL REFERENCES schools(id) ON DELETE CASCADE,
    domain VARCHAR(50) NOT NULL,
    reference_id UUID NOT NULL, -- e.g., requisition_id, journal_entry_id
    step VARCHAR(50) NOT NULL,
    actor_user_id UUID REFERENCES users(id),
    outcome VARCHAR(20) NOT NULL CHECK (outcome IN ('APPROVED', 'REJECTED', 'ESCALATED', 'OVERRIDE')),
    comments TEXT,
...
## 25. GOVERNMENT CAPITATION — RESTRICTED FUNDS

Capitation funds received from the MOE are ring-fenced by vote head (BRD RULE-FIN-003). Each disbursement is tracked and every drawdown must reference a journal line whose account belongs to the same vote head.

```sql
CREATE TABLE capitation_disbursements (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    school_id UUID NOT NULL REFERENCES schools(id) ON DELETE CASCADE,
    academic_year_id UUID NOT NULL REFERENCES academic_years(id),
    term_id UUID REFERENCES terms(id),
    vote_head_id UUID NOT NULL REFERENCES fee_vote_heads(id),
    reference VARCHAR(100) NOT NULL, -- MOE cheque or EFT reference
...
CREATE TABLE capitation_drawdowns (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    school_id UUID NOT NULL REFERENCES schools(id) ON DELETE CASCADE,
    disbursement_id UUID NOT NULL REFERENCES capitation_disbursements(id),
    vote_head_id UUID NOT NULL REFERENCES fee_vote_heads(id),
    amount DECIMAL(15,4) NOT NULL CHECK (amount > 0),
...
-- Aggregated remaining balance per disbursement.
CREATE OR REPLACE VIEW v_capitation_balances AS
SELECT
    ...
    COALESCE(SUM(dr.amount), 0) AS spent,
    d.amount - COALESCE(SUM(dr.amount), 0) AS remaining
FROM capitation_disbursements d
LEFT JOIN capitation_drawdowns dr ON dr.disbursement_id = d.id
GROUP BY d.id, d.school_id, d.academic_year_id, d.term_id, d.vote_head_id, d.amount;

-- Enforce vote-head alignment on drawdowns.
CREATE OR REPLACE FUNCTION check_capitation_vote_head_alignment()
RETURNS TRIGGER AS $$
DECLARE
    parent_vote_head UUID;
BEGIN
    SELECT vote_head_id INTO parent_vote_head
      FROM capitation_disbursements WHERE id = NEW.disbursement_id;
    IF parent_vote_head IS DISTINCT FROM NEW.vote_head_id THEN
        RAISE EXCEPTION 'Capitation drawdown vote_head must match its parent disbursement.';
    END IF;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;

CREATE TRIGGER capitation_vote_head_check
BEFORE INSERT OR UPDATE ON capitation_drawdowns
FOR EACH ROW EXECUTE FUNCTION check_capitation_vote_head_alignment();
```
