...
# Fernet key for PII columns; generate with: python -c "from cryptography.fernet import Fernet;print(Fernet.generate_key().decode())"
...
MPESA_EXPRESS_CALLBACK_URL=https://api.yourschool.co.ke/api/v1/webhooks/mpesa/stk-callback
MPESA_C2B_VALIDATION_URL=https://api.yourschool.co.ke/api/v1/webhooks/mpesa/c2b-validation
MPESA_C2B_CONFIRMATION_URL=https://api.yourschool.co.ke/api/v1/webhooks/mpesa/c2b-confirmation
# Comma-separated Safaricom source IPs allowed to call callbacks
MPESA_CALLBACK_ALLOWED_IPS=196.201.214.200,196.201.214.206,196.201.213.114
...
class Settings(BaseSettings):
    ...
    ENCRYPTION_KEY: str = Field(..., description="Fernet key for PII columns")
    ALGORITHM: str = "HS256"
...
> All validators use Pydantic v2 (`field_validator`, `ValidationInfo`). No v1 `@validator` remains. `Any` is imported from `typing`.
